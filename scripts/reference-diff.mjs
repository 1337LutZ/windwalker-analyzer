...
export function markdownFor(before, after) {
	...
	for (const row of rows) {
		...
		out.push(
			`| ${row.spec} | ${row.name} | ${n} | ${move(row.before.p50, row.after.p50)} | ${move(row.before.p90, row.after.p90)} |`,
		);
	}
	return out.join('\n');
}
...
function main() {
	...
	console.log(markdownFor(before, after));
}
