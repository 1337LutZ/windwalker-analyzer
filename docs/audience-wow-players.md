...
**Where this lives, and why here rather than with the skill it was written for.** It was measured as `references/audience-wow-players.md` under `.claude/skills/tone-of-voice/`, and that directory is ignored: the skill is an authoring aid for an agent working in this checkout, not something the repository ships. The measurements below are a different kind of thing — `docs/conventions.md` and `src/specs/__tests__/readerVoice.test.ts` cite them as the evidence for rules they enforce, and a citation a clone cannot open is a rule nobody can check. So the corpus record is tracked here, beside `docs/item-effect-sweep.md`, which is tracked and cited by a tracked test for exactly the same reason. The skill directory keeps its own copy for the agent that loads it; **if the two ever disagree, this one is the record.**
