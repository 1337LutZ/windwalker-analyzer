...
As of the split-group callout, it prints 1792 leaves, 851 prose, 26,467
words; median 27, p75 41, p90 60, p95 70, p99 102, max 158; longest 9% carry 23%; report.json 773 prose
leaves, 1,372 sentences, median 16, 242 past 25 (17.6%), 1 em-dash in 0 sentences (0.0%);
75 / 30 / 1 / 2 on the quote lines.

Four leaves, all of them the Lava Burst section's: a summary card for the free casts a Lava Surge hands
out, the row that says a surge expired inside an add phase, and the note that says why the tile counts
fewer than the table lists, written twice for its singular and plural arms. Three are prose and one is the
card's own label. The plural pair is why the sentence count moves by more than the prose leaf count, and
p90 came down a word, which is what four short strings do to a percentile taken over eight hundred. The
block before it was one leaf and no sentence, the cast log's tooltip label for an aura going up.
