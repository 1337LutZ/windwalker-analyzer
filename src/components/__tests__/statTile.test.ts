 ...
 * nothing a reader sees moves** — the rendered string is byte-for-byte what the concatenation
 * produced, which is why `elemental/components/sections/__tests__/unaskedVerdict.test.ts` still
 * asserts `Totem uptime — not measured at this many enemies` against the page and still passes.
 */
describe('StatTile caption', () => {
	...
	it('joins the caption to the label with the em dash the call sites used to write', () => {
	 ...
	 * the label exactly as it arrived — not `label — undefined`, and not a trailing dash on the several
