 ...
 *
 * `cleave` is the proof and it is committed: `good` over **14 of 14** points — nothing unread at all,
 * so (3) is not even needed — with `casts` and `debuff` both lettering `bad`. A reader of that pull was
 * told the two red sections below were not real mistakes.
 *
 * ---
 *
 ...
 * over every point its reading offered, with *two* of seven sections red rather than one. Its own
...
describe('the good headline does not deny the faults under it', () => {
	it('is printed over a pull one of whose sections it grades bad', () => {
		...
		expect(card.judged).toEqual({ measured: 14, total: 14, unmeasurable: false });
