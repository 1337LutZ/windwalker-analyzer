...
/**
 * The sticky bar's whole rendering of a pick-one choice: one button that opens the list.
 *
 * Neutral rather than picked, unlike the row classes above: it sits beside the Change and settings
 * buttons and is a way in to a choice rather than the choice itself, so colouring it as selected would
 * have it shouting louder than either of its neighbours. It keeps only the amber, which is not
 * decoration — a reading that contradicts what the pull detected has to be visible from the bar, and
 * collapsing a row of switches into one button is exactly where that could have been lost.
 *
 * There was a `toolbarChoiceClass` beside this until the bar stopped drawing that row at `md` and up:
 * the same palette at `px-1.5` and `min-w-11`, sized so three or four switches fit beside an encounter
 * name. `TargetModeToolbar` carries the measurement that retired it.
 *
