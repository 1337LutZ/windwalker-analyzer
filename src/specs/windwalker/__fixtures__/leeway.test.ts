...
// tests around `clampSetting` prove the number is safe, not that it changes anything.
import { describe, expect, it } from 'vitest';

import { defaultSettings } from '~/lib/settings';
import { analyse, WW_SETTINGS } from '~/specs/windwalker/lib';
...
describe.skipIf(token === '')('snapshot leeway against a live pull', () => {
	it('reclassifies snapshots without touching anything else', { timeout: 180_000 }, async () => {
		...
		const strict = analyse(dataset, { ...defaultSettings(WW_SETTINGS), snapshotLeewayMs: 1000 });
		const generous = analyse(dataset, { ...defaultSettings(WW_SETTINGS), snapshotLeewayMs: 2500 });
