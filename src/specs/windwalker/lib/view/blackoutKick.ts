...
function idOf(key: WW_AplRuleKey): number {
...
function ladderOf(apl: AplAudit | null | undefined): BlackoutKickLadder | null {
	...
	const counts = new Map<WW_AplRuleKey, number>();
	for (const press of skipped) {
		...
		counts.set(press.wanted as WW_AplRuleKey, (counts.get(press.wanted as WW_AplRuleKey) ?? 0) + 1);
	}
