...
import { energizingBrewPressable } from '~/specs/windwalker/lib';
...
const RJW_RULES = new Set<WW_AplRuleKey>(['rushing-jade-wind-open', 'rushing-jade-wind']);
...
function ladderOf(analysis: Analysis, apl: AplAudit | null | undefined): JadeWindLadder | null {
	...
	const decisions: JadeWindDecision[] = apl.presses
		.flatMap((press) => {
			// The engine's keys are plain strings; an audit produced from this ladder only ever carries
			// keys from `WW_AplRuleKey`.
			...
			if (press.pressed === RJW_CAST_ID) {
				rows.push({
					...
					wanted: press.wanted as WW_AplRuleKey,
					reason: press.reason ?? null,
				});
			}
			if (press.verdict === 'skipped' && press.wanted !== null && RJW_RULES.has(press.wanted as WW_AplRuleKey)) {
				rows.push({
					at: press.t,
					link: link?.(press.t) ?? null,
					kind: 'missed',
