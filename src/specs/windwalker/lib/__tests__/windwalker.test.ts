...
describe('analyse', () => {
	...
	it('reads the Tiger Palm refresh window from the settings', () => {
		...
		const reasonAt = (ms: number): string | undefined =>
			analyse(late, { ...defaultSettings(WW_SETTINGS), tigerPalmRefreshMs: ms }).filler.castList.find(
				(c) => c.t === 20500,
			)?.reason;

		expect(reasonAt(tigerPalm.default)).toBe('refresh');
		expect(reasonAt(tigerPalm.min)).toBe('wasted');
		...
		expect(analyse(late, { ...defaultSettings(WW_SETTINGS), tigerPalmRefreshMs: 3000 }).filler.refreshWindowSec).toBe(
			3,
		);
	});
	...
	it('reads the cooldown leeway from the settings, everywhere a wait is measured', () => {
		...
		const at = (ms: number) => analyse(held, { ...defaultSettings(WW_SETTINGS), cooldownLeewayMs: ms });
		...
		expect(kickRow(at(cooldown.min))?.driftSec).toBe(13);
		expect(kickRow(at(cooldown.default))?.driftSec).toBe(11.8);
		expect(at(cooldown.min).blackoutKick?.driftMs).toBe(13000);
		expect(at(cooldown.default).blackoutKick?.driftMs).toBe(11800);
	});
